...
def center_window(self, w, h):  # Pass in the tkinter frame (master) reference and the width and height
    ...
    # Calculate x and y coordinates to paint the app center on the user's screen
...
def addToList(self):
    ...
    var_fname = var_fname.title() # This will ensure that the character in each word is capitalized.
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname, var_lname)) # Combine our normalized names into a fullname.
    print("var_fullname: {}".format(var_fullname))
    var_phone = self.txt_phone.get().strip()
    var_email = self.txt_email.get().strip()
    if not '@' or not '.' in var_email: # We will use this soon.
        print("Incorrect email format!!!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # Enforce user to provide all
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            ...
            chkName = count
            if chkName == 0: # If this is zero then there is no fullname and we can add new data.
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # Check the count to ensure this is not the last record in
...
def onUpdate(self):
    try:
        ...
        messagebox.showinfo("Missing Selection", "No name was selected from the list box. \nCanceling the update request.")
        return
    # The user will only be allowed updates to phone number and email.
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):  # Ensure that there is data present.
        ...
        with conn:
            ...
            # the databasef...meaning, there are no changes to update.
            cur.execute("""SELECT COUNT(col_phone) FROM tbl_phonebook WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_phonebook WHERE col_email = '{}'""".format(var_phone))
